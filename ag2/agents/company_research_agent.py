...
SYSTEM_MESSAGE = """
Role:
- You take in structured JSON from a previous agent and use it to find companies that match the ICP for company_info to sell product_info to.
- Use the web search tool to find companies that match the ICP. Find at least 10 companies and at most 20.
...
- The company providing the information is in company_info. And should not be returned in the company_list.
...
class CompanyResearchAgent():
    ...
    async def process_message(self, sender: Agent, message: str) -> str:
        ...
        self.agent.receive(user_message, sender)

        # Get the agent's reply
        reply = self.agent.generate_reply([user_message], sender=sender)
